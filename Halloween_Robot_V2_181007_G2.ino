...
const int MS_OFF = 50;
const int OPEN_LOOP_DELAY = 3000;  // 3 secs - enough time for light show to complete
...
while (!Candy_Button_1_Untriggered && !LightShow_Active){
...
digitalWrite(LightShow_Interupt_Trigger_OUTPUT, LightShow_Interupt_Trigger_OUTPUT_State); // Send a pulse on the interupt, otherwise it locks out the other processor
delay (INTERUPT_PULSE);
...
Candy_Button_3_Untriggered = true; // REMOVE
...
// Start Serial to terminal for debugging
 ...
 attachInterrupt(Reset_InterBoard_INPUT, Reset_InterBoard_INPUT_Interupt, FALLING);  // FALLING = On Release
...
void Reset_InterBoard_INPUT_Interupt() {
